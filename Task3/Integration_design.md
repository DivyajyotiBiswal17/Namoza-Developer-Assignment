...
**Step 3 — WhatsApp Message via Karix**
...
The one edge case we can't fully resolve at the integration layer is if the same real patient uses two different phone numbers across two visits — they'll exist as two contacts in HubSpot. That's a data quality issue that would need to be resolved manually by the clinic coordinator, not something the integration can deduplicate on its own since we have no email or government ID as a secondary unique identifier
